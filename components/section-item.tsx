...
export default function SectionItem({
  ...
  const sectionTasks = tasks.filter((task) =>
    section.taskIds.includes(task.id),
  );
  ...
  return (
    <div className='space-y-3'>
      {/* Section Header */}
      <div
        className='flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-primary/10 to-accent/5 rounded-lg border-2 border-primary/30 hover:border-primary/50 transition-all group'
        onMouseEnter={() => setIsHovering(true)}
        onMouseLeave={() => setIsHovering(false)}
      >
        <button
          onClick={() => onToggleCollapse(section.id)}
          className='flex-shrink-0 p-1 hover:bg-primary/20 rounded transition-colors hover:cursor-pointer'
        >
          <ChevronDown
        ...
        {isHovering && (
          <div className='flex gap-2 ml-2'>
            <button
              ...
              className='p-2 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded transition-all hover:cursor-pointer'
            >
              <Edit2 className='w-4 h-4' />
            </button>
            <button
              onClick={() => onDeleteSection(section.id)}
              className='p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded transition-all hover:cursor-pointer'
            >
