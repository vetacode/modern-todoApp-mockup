...
export default function TaskInput({ onAddTask, sections }: TaskInputProps) {
  ...
  const selectedSectionTitle =
    sections.find((s) => s.id === selectedSection)?.title || 'Select section';
    ...
    <form onSubmit={handleSubmit} className='mb-6 sm:mb-8 space-y-3'>
      ...
      >
        ...
        <button
          type='submit'
          disabled={!input.trim()}
          className='px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed hover:cursor-pointer'
        >
          Add
        </button>
      </div>
      ...
      {sections.length > 0 && (
        <div className='relative'>
          <button
            ...
            className='w-full flex items-center justify-between px-4 py-2 bg-secondary/50 border-2 border-border rounded-lg hover:border-primary/50 transition-colors text-sm text-foreground hover:cursor-pointer'
          >
            <span className='text-sm'>
              Assign to:{' '}
              <span className='font-medium'>{selectedSectionTitle}</span>
            </span>
            <ChevronDown
